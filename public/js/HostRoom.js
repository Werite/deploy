...
const _PEER = {
  ...
  videoOn: '<i class="fas fa-regular fa-video"></i>',
  videoOff: '<i style="color: red;" class="fas fa-regular fa-video-slash"></i>',
  raiseHand:
    '<i style="color: rgb(0, 255, 71);" class="fas fa-hand-paper pulsate"></i>',
  lowerHand: "",
...
function initClient() {
  console.log("InitClient Room.js working");
  if (!DetectRTC.isMobileDevice) {
    ...
    setTippy("startRecButton", "Start Recording", "right");
    setTippy("stopRecButton", "Stop Recording", "right");
...
async function initEnumerateDevices() {
  ...
  if (!isAudioAllowed && !isVideoAllowed && !joinRoomWithoutAudioVideo) {
    openURL(
      `/permission?room_id=${room_id}&message=Not allowed both Audio and Video`
    );
  } else {
    hide(loadingDiv);
    getPeerGeoLocation();
...
function handleVideo(e) {
  ...
  e.target.className =
    "fas fa-regular fa-video " + (isVideoAllowed ? "" : "-slash");
}
...
function handleAudioVideo(e) {
  ...
  initAudioButton.className =
    "fas fa-microphone" + (isAudioVideoAllowed ? "" : "-slash");
  initVideoButton.className =
    "fas fa-regular fa-video" + (isAudioVideoAllowed ? "" : "-slash");
...
function joinRoom(peer_name, room_id) {
  ...
  fetch(`https://werite.in:5000/join/${room_id}/1`, {
    method: "PATCH",
    headers: {
  ...
  console.log("UPDATE INCALL Status of user.");
  let token;
  if (getCookie("localhost")) {
    const data = JSON.parse(getCookie("localhost"));
    token = data.token;
  ...
  fetch(`https://werite.in:5000/update/userincall/${room_id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      Authorization: token,
...
function displayPoolData({ title, category }) {
  ...
  containerElement.classList.add("pool_container");
  textElement.classList.add("pool_text");
  ...
  document.getElementById("pool_info").appendChild(containerElement);
}
...
function roomIsReady() {
  ...
  BUTTONS.main.acceptButton && show(acceptButton);
  BUTTONS.main.rejectButton && show(rejectButton);
...
function setupWhiteboardCanvasSize() {
  ...
  if (scaleFactorX < scaleFactorY && scaleFactorX < 1) {
    ...
    setWhiteboardSize(
      optimalSize[0] * scaleFactorX,
      optimalSize[1] * scaleFactorX
    );
    ...
    setWhiteboardSize(
      optimalSize[0] * scaleFactorY,
      optimalSize[1] * scaleFactorY
    );
  } else {
