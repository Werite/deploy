...
const html = {
  ...
  videoOn: "fas fa-regular fa-video",
  videoOff: "fas fa-regular fa-video-slash",
  userName: "username",
...
class RoomClient {
  constructor(
    ...
    this.isVideoFullScreenSupported =
      peer_info.is_mobile_device && peer_info.os_name === "iOS" ? false : true;
  ...
  async createRoom(room_id) {
    let recordingusername = window.localStorage.peer_name
  ...
  startLocalMedia() {
    ...
    console.log(
      "08 ----> Get Local Storage Devices before",
      localStorageDevices
    );
    if (localStorageDevices) {
      ...
      if (DEVICES_COUNT.audio != localStorageDevices.audio.count) {
        console.log(
          "08.1 ----> Audio devices seems changed, use default index 0"
        );
        microphoneSelect.selectedIndex = 0;
        this.setLocalStorageDevices(
          mediaType.audio,
          microphoneSelect.selectedIndex,
          microphoneSelect.value
        );
      }
      if (DEVICES_COUNT.speaker != localStorageDevices.speaker.count) {
        console.log(
          "08.2 ----> Speaker devices seems changed, use default index 0"
        );
        speakerSelect.selectedIndex = 0;
        this.setLocalStorageDevices(
          mediaType.speaker,
          speakerSelect.selectedIndex,
          speakerSelect.value
        );
      }
      if (DEVICES_COUNT.video != localStorageDevices.video.count) {
        console.log(
          "08.3 ----> Video devices seems changed, use default index 0"
        );
      ...
      console.log(
        "08.4 ----> Get Local Storage Devices after",
        this.getLocalStorageDevices()
      );
    }
    ...
    try {
      ...
      producer.on("transportclose", () => {
        ...
        if (!audio) {
          ...
          handleAspectRatio();
          console.log(
            "[transportClose] Video-element-count",
          ...
          console.log(
            "[transportClose] audio-element-count",
            this.localAudioEl.childElementCount
          );
        }
      ...
      producer.on("close", () => {
        ...
        if (!audio) {
          ...
          handleAspectRatio();
          console.log(
            "[closingProducer] Video-element-count",
            this.videoMediaContainer.childElementCount
          );
        } else {
          au.srcObject.getTracks().forEach(function (track) {
          ...
          au.parentNode.removeChild(au);
          console.log(
            "[closingProducer] audio-element-count",
            this.localAudioEl.childElementCount
          );
        }
        this.producers.delete(producer.id);
  ...
  async produceScreenAudio(stream) {
    try {
      ...
      if (this.producerLabel.has(mediaType.audio)) {
        return console.log(
          "Producer already exists for this type " + mediaType.audio
        );
      }
      ...
      const sa = await this.handleProducer(
        producerSa.id,
        mediaType.audio,
        stream
      );
  ...
  getCameraConstraints() {
    ...
    if (this.camera != "user")
      this.camVideo = { facingMode: { exact: this.camera } };
  ...
  async handleProducer(id, type, stream) {
    ...
    switch (type) {
      ...
      case mediaType.screen:
        ...
        this.isMobileDevice || type === mediaType.screen
          ? (elem.className = "")
          : (elem.className = "mirror");
        ...
        console.log(
          "[addProducer] Video-element-count",
          this.videoMediaContainer.childElementCount
        );
        break;
      case mediaType.audio:
        ...
        console.log(
          "[addProducer] audio-element-count",
          this.localAudioEl.childElementCount
        );
        break;
    }
  ...
  closeProducer(type) {
    ...
    if (type !== mediaType.audio) {
      ...
      handleAspectRatio();
      console.log(
        "[producerClose] Video-element-count",
        this.videoMediaContainer.childElementCount
      );
    }
    ...
    if (type === mediaType.audio) {
      ...
      console.log(
        "[producerClose] Audio-element-count",
        this.localAudioEl.childElementCount
      );
    }
  ...
  handleConsumer(id, type, stream, peer_name, peer_info) {
    ...
    switch (type) {
      ...
      case mediaType.screen:
        ...
        elem.className = "";
        elem.poster = image.poster;
        elem.style.objectFit = remoteIsScreen
          ? "contain"
          : "var(--videoObjFit)";
        vb = document.createElement("div");
        ...
        handleAspectRatio();
        console.log(
          "[addConsumer] Video-element-count",
          this.videoMediaContainer.childElementCount
        );
        if (!this.isMobileDevice) {
          this.setTippy(pn.id, "Toggle Pin", "top-end");
  ...
  removeConsumer(consumer_id, consumer_kind) {
    ...
    if (consumer_kind === "audio") {
      let audioConsumerPlayerId = this.getMapKeyByValue(
        this.audioConsumers,
        consumer_id
      );
  ...
  async setVideoOff(peer_info, remotePeer = false) {
    ...
    console.log(
      "[setVideoOff] Video-element-count",
      this.videoMediaContainer.childElementCount
    );
  }
  ...
  removeVideoOff(peer_id) {
    ...
    if (pvOff) {
      ...
      handleAspectRatio();
      console.log(
        "[removeVideoOff] Video-element-count",
        this.videoMediaContainer.childElementCount
      );
          ...
          .then((data) => {
            ...
            if (data.message === "Pool Empty") {
              fetch(`https://werite.in:5000/join/${pool_id}/0`, {
                ...
                .then((response) => response.json())
                .then((data) => {
                  console.log("Success isactive Pool status updated:", data);
                })
                .catch((error) => {
                  console.error(
                    "Error with pool isactive status update:",
                    error
                  );
                });
            }
            // console.log('Success pool empty:', data);
          });
      })
      .catch((error) => {
  ...
  handleFS(elemId, fsId) {
    ...
    if (btnFs) {
      ...
      btnFs.addEventListener("click", () => {
        videoPlayer.style.pointerEvents = this.isVideoOnFullScreen
          ? "auto"
          : "none";
  ...
  toggleChatEmoji() {
    ...
    this.getId("chatEmojiButton").style.color = this.isChatEmojiOpen
      ? "#FFFF00"
      : "#FFFFFF";
  }
  ...
  sendMessageTo(to_peer_id, to_peer_name) {
    if (!this.thereIsParticipants()) {
      ...
      return this.userLog(
        "info",
        "No participants in the room expect you",
        "top-end"
      );
    }
    Swal.fire({
  ...
  appendMessage(side, img, fromName, fromId, msg, toId, toName) {
    ...
    let replyMsg =
      fromId === this.peer_id ? `<hr/>Private message to ${toName}` : "";
  ...
  formatMsg(message) {
    ...
    if (this.isValidHttpURL(message)) {
      if (isImageURL(message))
        return (
          '<img src="' + message + '" alt="img" width="180" height="auto"/>'
        );
      if (this.isVideoTypeSupported(message)) return this.getIframe(message);
      return (
        '<a href="' +
        message +
        '" target="_blank" class="msg-a">' +
        message +
        "</a>"
      );
    }
  ...
  startRecording() {
    ...
    try {
      if (this.isMobileDevice) {
        ...
        let newStream = this.getNewStream(
        ...
        navigator.mediaDevices
          ...
          .then((screenStream) => {
            this.recScreenStream = this.getNewStream(
              screenStream,
              this.localAudioStream
            );
            this.mediaRecorder = new MediaRecorder(
              this.recScreenStream,
              options
            );
            console.log(
              "Created MediaRecorder",
          ...
          .catch((err) => {
            ...
            this.userLog(
              "error",
              "Unable to recording the screen + audio reason: " + err,
              "top-end"
            );
          });
      }
      ...
      return this.userLog(
        "error",
        "Can't start stream recording reason: " + err,
        "top-end"
      );
    }
  }
  ...
  getNewStream(videoStream, audioStream) {
    let newStream = null;
    let videoStreamTrack = videoStream
      ? videoStream.getVideoTracks()[0]
      : undefined;
    let audioStreamTrack = audioStream
...
`;
      ...
      if (uploadVideoToS3) {
        uploadVideoToS3.addEventListener("click", async () => {
          ...
          const { vidRecURL } = await fetch(`/s3Url/${recordingname}`).then(
            (res) => res.json()
          );
    ...
    if (videoPlayer) {
      ...
      videoPlayer.addEventListener("drop", function (e) {
        ...
        if (e.dataTransfer.items && e.dataTransfer.items.length > 1) {
          return userLog(
            "warning",
            "Please drag and drop a single file.",
            "top-end"
          );
        }
  ...
  sendFileData(peer_id, broadcast) {
    ...
    this.fileReader.addEventListener("error", (err) =>
    ...
    this.fileReader.addEventListener("load", (e) => {
      ...
      sendFilePercentage.innerHTML =
        "Send progress: " +
        ((offset / this.fileToSend.size) * 100).toFixed(2) +
        "%";
      ...
      if (offset === this.fileToSend.size) {
        ...
        userLog(
          "success",
          "The file " + this.fileToSend.name + " was sent successfully.",
          "top-end"
        );
      }
  ...
  handleFileAbort(data) {
    ...
    userLog(
      "info",
      data.peer_name + " ⚠️ aborted the file transfer",
      "top-end"
    );
  }
  ...
  shareVideo(peer_id = "all") {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        let video_url = is_youtube
          ? this.getYoutubeEmbed(result.value)
          : result.value;
        if (video_url) {
          let data = {
            peer_id: peer_id,
  ...
  shareVideoAction(data) {
    ...
    switch (action) {
      case "open":
        this.userLog(
          "info",
          `${peer_name} <i class="fab fa-youtube"></i> opened the video`,
          "top-end"
        );
        this.openVideo(data);
        break;
      case "close":
        this.userLog(
          "info",
          `${peer_name} <i class="fab fa-youtube"></i> closed the video`,
          "top-end"
        );
        this.closeVideo();
        break;
  ...
  closeVideo(emit = false, peer_id = "all") {
    ...
    if (shareVideoDiv) {
      ...
      handleAspectRatio();
      console.log(
        "[closeVideo] Video-element-count",
        this.videoMediaContainer.childElementCount
      );
      this.sound("left");
    }
    ...
    switch (data.lobby_status) {
      case "waiting":
        if (!isRulesActive || isPresenter) {
          ...
          lobbyHeaderTitle.innerText =
            "Lobby users (" + lobbyParticipantsCount + ")";
          ...
          this.userLog(
            "info",
            peer_name + " wants to join the meeting",
            "top-end"
          );
        }
        break;
  ...
  peerAction(
    ...
    if (emit) {
      ...
      if (!this.thereIsParticipants()) {
        if (info)
          return this.userLog("info", "No participants detected", "top-end");
      }
  ...
  confirmPeerAction(action, data) {
    switch (action) {
      case "eject":
        ...
        let whoEject = data.broadcast
          ? "All participants except yourself?"
          : "current participant?";
        Swal.fire({
          background: swalBackground,
          position: "center",
      ...
      case "hide":
        ...
        let whoMuteHide = data.broadcast
        ...
        Swal.fire({
          ...
          title:
            action == "mute" ? "Mute " + whoMuteHide : "Hide " + whoMuteHide,
          text:
            action == "mute"
          ...
          .then((result) => {
            if (result.isConfirmed) {
              ...
              if (!data.broadcast) {
                ...
                switch (action) {
                  case "mute":
                    let peerAudioButton = this.getId(
                      data.peer_id + "___pAudio"
                    );
                    if (peerAudioButton)
                      peerAudioButton.innerHTML = _PEER.audioOff;
                    break;
                  case "hide":
                    let peerVideoButton = this.getId(
                      data.peer_id + "___pVideo"
                    );
                    if (peerVideoButton)
                      peerVideoButton.innerHTML = _PEER.videoOff;
                }
              } else {
                this.socket.emit("peerAction", data);
                let actionButton = this.getId(action + "AllButton");
  ...
  setLocalStorageDevices(type, index, select) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(LOCAL_STORAGE_DEVICES)
    );
  }
