...
notificationSocket.use(async function(socket, next){
    ...
    if (hostCfg.authenticated) {
        ...
        if(!isUserHost)
        {
            ...
            if (!isCurrentPoolActive) {
                res.status(404).json({message:"Pool Call Currently Not Active"});
                return;
            }
            ...
            if (!isUserAllowedToJoinPool) {
                res.status(404).json({message : "Not authorised to join this call"});
                return;
            }
            res.sendFile(views.room);
            return ;
...
app.get('/join/pool/:id', protect,async (req, res) => {
    ...
    if (!isUserAllowedToJoinPool) {
        ...
        res.status(200).json({message : "Not authorised to join this call"});
        return;
    }
    if (!isCurrentPoolActive) {
    ...
    res.status(200).json({message:"Call Active"});
});
...
app.get('/isuserincall', protect, async (req, res) => {
    ...
    if (isUserAlreadyInACall) {
        ...
        res.status(200).json({message:"You can join another call"});
        return;
    }
})
