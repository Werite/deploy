...
const createPoolRequest = async(req, res,next) => {
    try {
        ...
        let hasPoolRequest = await PoolRequest.findOne(req.user.user_id , req.params.id);
        ...
        let poolRequest = new PoolRequest(stance,guts, req.user.user_id , req.params.id);
        ...
        let notification = new Notification(`${req.user.username} sent you a Request for the pool ${poolData.title}`,poolData.user_id);
        ...
        return res.status(201).json({ message: "Request Sent Successfully!", noti: message });
...
const getMyPoolRequests = async(req, res,next) => {
  try {
      ...
      PoolRequests= await Promise.all(PoolRequests.map(async(req)=>{
...
const acceptPoolRequest = async (req, res) => {
  try {
    ...
    if(!isUserHost)
    {
      return res.status(403).json({ message: "Permission Denied! Not a host." })
    }
    
    await PoolRequest.acceptPoolRequest(pool_id, joiningUser_id)
    let user = await User.findById(isUserHost.user_id);
    user= user[0][0]
    let notification = new Notification(`${user.username} accepted your Request for the pool ${isUserHost.title}`,joiningUser_id);
...
const rejectPoolRequest = async (req, res) => {
  try {
    ...
    if(!isUserHost)
    {
      return res.status(404).json({ message: "Permission Denied! Not a host." })
    }
